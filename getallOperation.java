...
public class getallOperation {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					getallOperation window = new getallOperation();
	...
	void handle_query() {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         while(rset.next()) { 
	            ...
	            name.add(rset.getString("opname"));
	            	            description.add(rset.getString("description"));
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"OPID", "Name", "Description"});
		//System.out.println("Afafd");
		i=0;
		while(i<opid.size() && i<10)
		{
			model.addRow(new Object[] {opid.get(i), name.get(i), description.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
			//model.addRow(new Object[] {opid.get(i), name.get(i), phone_no.get(i)});
	}
	...
	private void initialize() {
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(i>opid.size())
					return;
				while(i<opid.size() && i<(present+1)*10)
				{
					model.addRow(new Object[] {opid.get(i), name.get(i), description.get(i)});
					i++;
				}
		...
		JLabel lblAllPatientsInformation = new JLabel("ALL OPERATION INFORMATION");
