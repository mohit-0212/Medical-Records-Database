...
public class getallBills {
	...
	public ArrayList<String> bdate = new ArrayList<>();
		
	public ArrayList<String> payment_mode = new ArrayList<>();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					getallBills window = new getallBills();
	...
	void handle_query() {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         while(rset.next()) { 
	            ...
	            bname.add(rset.getString("bname"));
	            bdate.add(rset.getString("bdate"));
	            
	            	            payment_mode.add(rset.getString("payment_mode"));
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"BNO", "Name", "Bill Date", "Payment Mode"});
		//System.out.println("Afafd");
		i=0;
		while(i<bno.size() && i<10)
		{
			model.addRow(new Object[] {bno.get(i), bname.get(i), bdate.get(i), payment_mode.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
	...
	private void initialize() {
		...
		JLabel lblAllPatientsInformation = new JLabel("ALL BILLS INFORMATION");
