...
public class get_bill_past {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					get_bill_past window = new get_bill_past();
	...
	void handle_query(String input) {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         while(rset.next()) { 
	        	//Integer id = new Integer(rset.getInt("bno"));
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"PID", "Bill no."});
		//System.out.println("Afafd");
		i=0;
		while(i<bno.size() && i<10)
		{
			model.addRow(new Object[] {abc, bno.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//for(i=present*10;i<(present+1)*10;i++)
					//model.addRow(new Object[] {bno.get(i), name.get(i), complain.get(i), date_admitted.get(i), date_discharged.get(i), birthdate.get(i)});
				if(i<bno.size())
				present++;
			}
		});
