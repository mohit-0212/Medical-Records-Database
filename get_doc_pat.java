...
public class get_doc_pat {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					get_doc_pat window = new get_doc_pat();
	...
	void handle_query(String input) {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         while(rset.next()) { 
	        	//Integer id = new Integer(rset.getInt("did"));
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"PID", "DID", "Date"});
		//System.out.println("Afafd");
		i=0;
		while(i<did.size() && i<10)
		{
			model.addRow(new Object[] {abc, did.get(i), consult_date.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
			//model.addRow(new Object[] {did.get(i), name.get(i), phone_no.get(i)});
	}
	...
	private void initialize() {
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(i>did.size())
					return;
				while(i<did.size() && i<(present+1)*10)
				{
					model.addRow(new Object[] {abc, did.get(i), consult_date.get(i)});
					i++;
				}
				//for(i=present*10;i<(present+1)*10;i++)
					//model.addRow(new Object[] {did.get(i), name.get(i), complain.get(i), date_admitted.get(i), date_discharged.get(i), birthdate.get(i)});
				if(i<did.size())
				present++;
			}
		});
