...
public class get_history {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					get_history window = new get_history();
	...
	void handle_query(String input) {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         while(rset.next()) { 
	        	//Integer id = new Integer(rset.getInt("diid"));
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"PID", "DIID"});
		//System.out.println("Afafd");
		i=0;
		while(i<diid.size() && i<10)
		{
			model.addRow(new Object[] {abc, diid.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//for(i=present*10;i<(present+1)*10;i++)
					//model.addRow(new Object[] {diid.get(i), name.get(i), complain.get(i), date_admitted.get(i), date_discharged.get(i), birthdate.get(i)});
				if(i<diid.size())
				present++;
			}
		});
