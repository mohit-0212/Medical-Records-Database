...
public class get_testcommon {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					get_testcommon window = new get_testcommon();
	...
	void handle_query() {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         String query = "select distinct(te.tid), te.name, di.diid from (select t.tid, dis.diid from (select num.diid, cnt from (select diid, count(pid) as cnt from has group by diid) as num where cnt = (select max(cnt2) from (select diid, count(pid) as cnt2 from has group by diid) as num2)) as dis, has h, takes_t t where dis.diid = h.diid and h.pid = t.pid) as di, tests te where di.tid = te.tid";
	         ResultSet rset = stmt.executeQuery(query);
	         while(rset.next()) { 
	        	//Integer id = new Integer(rset.getInt("tid"));
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"TID", "Name", "DIID"});
		//System.out.println("Afafd");
		i=0;
		while(i<tid.size() && i<10)
		{
			model.addRow(new Object[] {tid.get(i), test_name.get(i), diid.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//for(i=present*10;i<(present+1)*10;i++)
					//model.addRow(new Object[] {tid.get(i), name.get(i), complain.get(i), date_admitted.get(i), date_discharged.get(i), birthdate.get(i)});
				if(i<tid.size())
				present++;
			}
		});
