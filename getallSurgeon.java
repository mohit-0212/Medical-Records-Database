...
public class getallSurgeon {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					getallSurgeon window = new getallSurgeon();
	      ...
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         while(rset.next()) { 
	            ...
	            did.add(rset.getString("did"));
	            	            specialization.add(rset.getString("specialization"));
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"SID", "DID", "Specialization"});
		//System.out.println("Afafd");
		i=0;
		while(i<sid.size() && i<10)
		{
			model.addRow(new Object[] {sid.get(i), did.get(i), specialization.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
			//model.addRow(new Object[] {sid.get(i), name.get(i), phone_no.get(i)});
	}
	
	public getallSurgeon() {
		initialize();
		handle_query();
		disp();
	}
	...
	private void initialize() {
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(i>sid.size())
					return;
				while(i<sid.size() && i<(present+1)*10)
				{
					model.addRow(new Object[] {sid.get(i), did.get(i), specialization.get(i)});
					i++;
				}
		...
		JLabel lblAllPatientsInformation = new JLabel("ALL SURGEON INFORMATION");
