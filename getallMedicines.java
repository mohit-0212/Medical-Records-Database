...
public class getallMedicines {
	...
	public ArrayList<String> stock = new ArrayList<>();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					getallMedicines window = new getallMedicines();
	...
	void handle_query() {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         while(rset.next()) { 
	            ...
	            price.add(rset.getString("price"));
	            
	            stock.add(rset.getString("stock"));
	            	         }	 
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"MID", "Name", "Price", "Stock"});
		//System.out.println("Afafd");
		i=0;
		while(i<mid.size() && i<10)
		{
			model.addRow(new Object[] {mid.get(i), name.get(i), price.get(i), stock.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
	...
	private void initialize() {
		...
		JLabel lblAllPatientsInformation = new JLabel("ALL MEDICINE INFORMATION");
