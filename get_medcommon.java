...
public class get_medcommon {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					get_medcommon window = new get_medcommon();
	...
	void handle_query() {
	      try (Connection conn = DriverManager.getConnection(
	         ...
	         String query = "select m.mid, m.name, dis.diid from (select num.diid, cnt from (select diid, count(pid) as cnt from has group by diid) as num where cnt = (select max(cnt2) from (select diid, count(pid) as cnt2 from has group by diid) as num2)) as dis, cures c, medicine m  where dis.diid = c.diid and c.mid = m.mid";
	...
	void disp()
	{
		...
		model.setRowCount(0);
		model.addRow(new Object[]{"MID", "Name", "DIID"});
		//System.out.println("Afafd");
		i=0;
		while(i<mid.size() && i<10)
		{
			model.addRow(new Object[] {mid.get(i), med_name.get(i), diid.get(i)});
			i++;	
		}
		//for(i=0;i<10;i++)
